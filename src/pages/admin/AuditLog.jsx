    ...
    return (
        <div className="space-y-6 fade-in">
            <div>
                ...
                <p className="text-slate-500 dark:text-dark-600 text-sm mt-1">Registro histórico de todas las acciones del sistema</p>
            </div>
                ...
                <div className="flex gap-2">
                    {types.map(t => (
                        <button key={t} onClick={() => setTypeFilter(t)}
                            className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${typeFilter === t ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-dark-200 text-slate-600 dark:text-dark-700 hover:bg-slate-200 dark:hover:bg-dark-300'}`}>
            ...
            <div className="glass-card divide-y divide-dark-300/50">
                {logs.length === 0 ? (
                    <div className="p-8 text-center text-slate-400 dark:text-dark-600">No hay registros que coincidan</div>
                ) : (
                    logs.map(log => (

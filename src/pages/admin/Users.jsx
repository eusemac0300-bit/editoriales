        ...
        <div className="space-y-6 fade-in">
            <div className="flex flex-col sm:flex-row justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-dark-600 text-sm mt-1">{staffCount} cuentas de personal registradas</p>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {ROLES.map(role => (
                    <div
                        key={role}
                        className={`stat-card text-center cursor-pointer transition-all ${filter === role ? 'ring-2 ring-primary' : 'hover:ring-1 hover:ring-slate-300 dark:hover:ring-dark-400'}`}
                        onClick={() => setFilter(filter === role ? 'TODOS' : role)}
                    >
                        <Shield className={`w-5 h-5 mx-auto mb-2 ${role === 'ADMIN' ? 'text-primary' : role === 'VENDEDOR' ? 'text-amber-500 dark:text-yellow-400' : role === 'EDITOR' ? 'text-purple-600 dark:text-purple-400' : 'text-emerald-600 dark:text-emerald-400'}`} />
                        <p className="text-2xl font-bold text-slate-900 dark:text-white">{data.users.filter(u => u.role === role).length}</p>
                        <p className="text-[10px] text-slate-500 dark:text-dark-600 uppercase">{ROLE_LABELS[role]}s</p>
                    </div>
                ))}
            </div>
            ...
            {filter !== 'TODOS' && (
                <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-500 dark:text-dark-600">Filtrado por:</span>
                    <span className={ROLE_COLORS[filter]}>{ROLE_LABELS[filter]}</span>
                    <button onClick={() => setFilter('TODOS')} className="text-xs text-slate-400 dark:text-dark-500 hover:text-primary transition-colors">
                        <X className="w-3 h-3" />
                    </button>
                </div>
            )}
            ...
            <div className="glass-card divide-y divide-dark-300/50">
                {filteredUsers.map(u => (
                    <div key={u.id} className="flex items-center gap-4 p-4 hover:bg-slate-50 dark:hover:bg-dark-200/30 transition-colors group">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold text-white shrink-0 ${u.role === 'ADMIN' ? 'bg-gradient-to-br from-primary to-primary-700' : u.role === 'VENDEDOR' ? 'bg-gradient-to-br from-amber-500 to-amber-600' : u.role === 'EDITOR' ? 'bg-gradient-to-br from-purple-500 to-purple-600' : 'bg-gradient-to-br from-emerald-500 to-emerald-700'}`}>
                            {u.avatar}
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-slate-900 dark:text-white">{u.name}</p>
                            <p className="text-xs text-slate-500 dark:text-dark-600">{u.email} · {u.title}</p>
                        </div>
                        ...
                        <div className="flex items-center gap-1 opacity-1 group-hover:opacity-100 transition-opacity">
                            <button
                                ...
                                className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-dark-200 text-slate-400 dark:text-dark-600 hover:text-primary transition-all"
                            ...
                            {u.id !== user?.id && (
                                <button
                                    ...
                                    className="p-2 rounded-lg hover:bg-red-500/10 text-slate-400 dark:text-dark-600 hover:text-red-500 dark:hover:text-red-400 transition-all"
                ...
                <div className="fixed inset-0 z-[80] bg-black/70 flex items-center justify-center p-4" onClick={() => setDeleteConfirm(null)}>
                    <div className="glass-card max-w-sm w-full p-6 slide-up" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Eliminar Usuario</h3>
                                <p className="text-xs text-slate-500 dark:text-dark-600">Esta acción no se puede deshacer</p>
                            </div>
                        </div>
                        <p className="text-sm text-slate-600 dark:text-dark-800 mb-4">
            ...
            <div className="glass-card p-5">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Matriz de Permisos</h2>
                        {saved && (
                            <span className="text-[10px] text-emerald-600 dark:text-emerald-400 flex items-center gap-1 fade-in">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1 text-[10px] text-slate-400 dark:text-dark-500 hover:text-primary transition-colors px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-dark-200"
                        title="Restaurar permisos por defecto"
                    >
                        <RotateCcw className="w-3 h-3" /> Restablecer
                    </button>
                </div>
                <p className="text-[10px] text-slate-500 dark:text-dark-500 mb-3">Haz clic en cualquier celda para cambiar el permiso</p>
                <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                        ...
                        <tbody>
                            {MODULES.map((mod, i) => (
                                <tr key={i} className="border-b border-slate-200 dark:border-dark-300/30 hover:bg-slate-50 dark:hover:bg-dark-200/20 transition-colors">
                                    <td className="py-2 px-3 text-slate-700 dark:text-dark-800 font-medium">{mod}</td>
                                    {ROLES.map(role => {
                                            ...
                                            <td key={role} className="py-2 px-3 text-center">
                                                <button
                                                    ...
                                                    className={`w-7 h-7 rounded-lg inline-flex items-center justify-center transition-all duration-200 ${allowed
                                                        ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/25 ring-1 ring-emerald-500/20'
                                                        : 'bg-red-500/10 text-red-500 dark:text-red-400/60 hover:bg-red-500/20 ring-1 ring-red-500/10'
                                                        }`}

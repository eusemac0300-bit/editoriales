                    ...
                    <div className="flex items-center gap-3 p-1 bg-primary/20 rounded-2xl border-2 border-primary shadow-xl shadow-primary/20 backdrop-blur-xl animate-in zoom-in-95 duration-500 scale-105 mr-4 mr-2">
                        <div className="px-4 py-2 border-r border-primary/20">
                            ...
                            <p className="text-xs font-black text-slate-800 dark:text-white-100 flex items-center gap-2 mt-0.5">
                                ...
                                v3.1.5.11 LISTA PARA LIBERAR
                            </p>
                        </div>
                        <button
                            disabled={isPublishing}
                            onClick={async () => {
                                if (window.confirm('¿Publicar la v3.1.5.11 para todas las editoriales ahora? Esto notificará a todos tus clientes.')) {
                                    setIsPublishing(true)
                                    try {
                                        await publishAppVersion('v3.1.5.11', ['Universal Cache Breaker', 'Fuerza v11', 'Limpieza Vercel'])
                                        alert('¡ÉXITO! Versión v3.1.5.11 liberada globalmente. Se ha notificado a todas las editoriales.')
                                    } catch (err) {

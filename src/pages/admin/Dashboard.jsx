...
export default function AdminDashboard() {
    ...
    const stats = [
        { label: 'Libros Totales', value: totalBooks, icon: BookOpen, color: 'from-primary to-primary-700', change: `${published} publicados` },
        { label: 'En Producción', value: inProduction, icon: TrendingUp, color: 'from-amber-500 to-amber-700', change: `${books.filter(b => b.status === 'Original').length} manuscritos` },
        { label: 'Ingresos', value: formatCLP(totalIncome), icon: DollarSign, color: 'from-emerald-500 to-emerald-700', change: '+12% vs período ant.', up: true },
        { label: 'Egresos', value: formatCLP(totalExpenses), icon: ArrowDownRight, color: 'from-red-500 to-red-700', change: 'Costos acumulados' },
        { label: 'Stock Físico', value: `${totalStock} uds.`, icon: Package, color: 'from-purple-500 to-purple-700', change: `${inventory.physical.filter(p => p.stock < p.minStock).length} bajo mín.` },
        { label: 'Alertas Activas', value: criticalAlerts, icon: AlertTriangle, color: 'from-orange-500 to-orange-700', change: 'Requieren atención' },
    ]
    ...
    return (
        <div className="space-y-6 fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {stats.map((stat, i) => (
                    <div key={i} className="stat-card slide-up" style={{ animationDelay: `${i * 0.05}s` }}>

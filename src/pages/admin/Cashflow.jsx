    ...
    return (
        <div className="space-y-6 fade-in pb-10">
            ...
            <div>
                ...
                <p className="text-dark-600 text-sm mt-1">Proyección de liquidez basada en compromisos pendientes</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="stat-card border-primary/20 bg-primary/5">
                    ...
                    <p className="text-2xl font-bold text-white font-mono">{formatCLP(calculations.receivables)}</p>
                    <p className="text-[10px] text-dark-600 mt-2 flex items-center gap-1">
                ...
                <div className="stat-card border-orange-500/20 bg-orange-500/5">
                    ...
                    <p className="text-2xl font-bold text-white font-mono">{formatCLP(calculations.payables)}</p>
                    <p className="text-[10px] text-dark-600 mt-2">Regalías aprobadas + OC pendientes</p>
                </div>

                <div className={`stat-card ${calculations.netBalance >= 0 ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-red-500/20 bg-red-500/5'}`}>
                    <p className="text-[10px] text-dark-500 uppercase font-black tracking-widest mb-1">Balance Proyectado</p>
                    ...
                    <p className="text-[10px] text-dark-600 mt-2">Diferencia neta de liquidez</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-card p-6">
                    ...
                    <div className="space-y-4">
                        <div className="p-4 bg-dark-200/50 rounded-xl border border-dark-300 flex justify-between items-center group hover:border-primary/30 transition-all">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-lg bg-orange-500/10 flex items-center justify-center">
                                    <Users className="w-5 h-5 text-orange-400" />
                                </div>
                                <div>
                                    <p className="text-sm font-bold text-white">Regalías de Autores</p>
                                    <p className="text-xs text-dark-600">Aprobadas para liquidación</p>
                                </div>
                            </div>
                            <p className="text-lg font-bold text-white font-mono">{formatCLP(calculations.details.royalties)}</p>
                        </div>
                        ...
                        <div className="p-4 bg-dark-200/50 rounded-xl border border-dark-300 flex justify-between items-center group hover:border-primary/30 transition-all">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-white">Producción en Curso</p>
                                    <p className="text-xs text-dark-600">Órdenes de compra abiertas</p>
                                </div>
                            </div>
                            <p className="text-lg font-bold text-white font-mono">{formatCLP(calculations.details.pos)}</p>
                        </div>

                        <div className="mt-6 pt-6 border-t border-dark-300/50 flex items-center gap-3 text-dark-600 italic text-sm">
                ...
                <div className="glass-card p-6">
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between text-xs text-dark-600 mb-2">
                        ...
                        {consignments.filter(c => c.status !== 'FINALIZADO').slice(0, 5).map(c => (
                            <div key={c.id} className="flex justify-between items-center p-3 border-b border-dark-300/30">
                                <div>
                                    <p className="text-sm font-medium text-white">{c.clientName}</p>
                                    <p className="text-[10px] text-dark-600 uppercase font-bold tracking-tighter">{c.bookTitle}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-bold text-emerald-400 font-mono">
                                        {formatCLP((c.sentQuantity - c.returnedQuantity - c.soldQuantity) * 6000)}
                                    </p>
                                    <p className="text-[10px] text-dark-500">{c.sentQuantity - c.returnedQuantity - c.soldQuantity} unid.</p>
                                </div>
                            </div>
                        ))}

                        {consignments.length === 0 && (
                            <div className="py-10 text-center text-dark-600 italic text-sm">
